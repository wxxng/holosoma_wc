...
export LD_LIBRARY_PATH=${LD_LIBRARY_PATH}:${CONDA_ROOT}/envs/holomujoco_mw/lib

# Validate environment is properly activated
PYBIN=/home/rllab3/.holosoma_deps/miniconda3/envs/holomujoco_mw/bin/python
if ${PYBIN} -c "import mujoco" 2>/dev/null; then
    echo "MuJoCo environment activated successfully (holosoma_mw)"
