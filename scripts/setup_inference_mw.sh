...
ENV_ROOT=$CONDA_ROOT/envs/holoinference_mw
...
if [[ ! -f $SENTINEL_FILE ]]; then
  ...
  if [[ ! -d $ENV_ROOT ]]; then
    ...
    MAMBA_ROOT_PREFIX=$CONDA_ROOT $CONDA_ROOT/bin/mamba create -y -n holoinference_mw python=3.10 -c conda-forge --override-channels
  fi

  source $CONDA_ROOT/bin/activate holoinference_mw
